    ...
    try {
      ...
      if (basicData.name !== "") {
        await docRef
          ...
          .then(dispatch(updateDisplayname(basicData.name, id)))
          .then(toastr.success("success", "name updated"))
          .catch((err) => {
      ...
      if (basicData.gender !== null) {
        await docRef
          ...
          .then(toastr.success("success", "gender info noted"));
      }
      if (basicData.birthday !== "") {
        ...
        await docRef
          ...
          .then(toastr.success("success", "birthday info updated"));
      }
    } catch (error) {}
  };
};
...
export const uploadPhoto = (file, userId, fileName) => {
  return async (dispatch) => {
    ...
    try {
      ...
      let downloadUrl;
      await userImageRef.put(file, metadata).then((res) =>
    ...
    try {
      ...
      if (userMainPhoto === undefined) {
        await firestore
          ...
          .then(dispatch(mainPhotoUpdate(photo)));
      }
      if (userMainPhoto !== undefined) {
        await firestore
          .doc(`user/${userId}`)
          .update({ mainPhoto: photo })
          .then(dispatch(mainPhotoUpdate(photo)));
      }
    } catch (error) {
      console.error(error);
  ...
  return async (dispatch) => {
    ...
    try {
      ...
      if (dataInterest === undefined) {
        ...
        if (!compareArray(dataInterest, interest)) {
          task = "interest";
          info = interest;
          await firestore
            .doc(`user/${id}`)
...
export const addfollowerFinish = () => {
  return {
    type: "ADD_FOLLOWER",
  };
};

export const addfollower = (people, follower) => {
  return async (dispatch) => {
    try {
      await firestore.doc(`user/${people}`).update({
        follower: firebase.firestore.FieldValue.arrayUnion(follower),
      });
      await firestore.doc(`user/${follower}`).update({
        following: firebase.firestore.FieldValue.arrayUnion(people),
      });
      const followerName = await firestore
        .doc(`user/${follower}`)
        .get()
        .then((doc) => {
...
export const unFollow = (people, follower) => {
  return async (dispatch) => {
    try {
      await firebase
        .firestore()
        .doc(`user/${people}`)
        .update({
          follower: firebase.firestore.FieldValue.arrayRemove(follower),
        });
      await firebase
        .firestore()
        .doc(`user/${follower}`)
        .update({
          following: firebase.firestore.FieldValue.arrayRemove(people),
        });
      dispatch(addfollowerFinish());
    } catch (error) {
      console.log(error);
    }
  };
