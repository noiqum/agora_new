...
export class photo extends Component {
  ...
  render() {
    ...
    const profilePhotoHolder = () => {
      return profilePhoto !== undefined
        ? profilePhoto
        : {
            dowvloadUrl:
              "gs://agora-event-platform.appspot.com/userPlaceHolder.png",
          };
    };
    return (
      <div ref={this.photoRef} className="photo">
        <h2>Photos</h2>
        ...
        <div className="photo__part-3">
          ...
          {this.state.files[0] && (
            <img src={this.state.cropResult} alt="dropped_image" />
          )}
          {this.state.files[0] && (
            <div className="photo__part-3__upload" onClick={this.uploadImage}>
              Upload
            </div>
          )}
        </div>
            ...
            photos.map((photo, index) => {
              return (
                <PhotoItem
                  ...
                  onMainClick={() => {
                    this.mainHandle(photo);
                  }}
                  onDeleteClick={() => {
                    this.deleteHandle(photo);
                  }}
                />
              );
            })}
...
const mapDispatchToProps = (dispatch) => {
  return {
    onUploadImage: (file, userId, fileName) => {
      dispatch(uploadPhoto(file, userId, fileName));
    },
    onMain: (pic, id) => {
      dispatch(mainPhotoPick(pic, id));
    },
    onDelete: (photo, id, fileName) => {
      dispatch(deletePhoto(photo, id, fileName));
    },
  };
};
